...
import {
  CalendarDays, Users, Inbox, AlertCircle, Check, Loader2, CheckCircle2, ChevronDown, Lock,
} from "lucide-react";
...
function AttendanceCell({ status, onSet, locked = false }) {
  ...
  if (locked) {
    return (
      <Box
        className={cn(
          "mx-auto flex h-8 w-9 items-center justify-center rounded-lg text-xs font-bold ring-1 opacity-80",
          cfg ? cfg.cell : UNMARKED_CELL
        )}
        title={`${cfg ? cfg.label : "Unmarked"} — locked`}
      >
        {cfg ? cfg.short : "–"}
      </Box>
    );
...
function AttendanceGrid({ token, trainingRef }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMarks(seeded);
      setDirty(new Set());
      setData({ sessions, participants: detail.participants || [], trainingStatus: detail.status });
    } catch (e) {
  ...
  const { sessions, participants, trainingStatus } = data;
  // Mirrors ATTENDANCE_LOCKED_STATUSES on the server — an admin marking the
  // training completed (or cancelled) closes the register. The API rejects the
  // write either way; this just stops a trainer making edits that can't be saved.
  const locked = trainingStatus === "completed" || trainingStatus === "cancelled";
              ...
              <Box as="tr" key={p.participant_id} className={cn("border-b border-slate-50", i % 2 ? "bg-slate-50/40" : "bg-white")}>
                ...
                {sessions.map((s) => (
                  <Box as="td" key={s.id} className="px-2 py-2.5 text-center">
                    <AttendanceCell
                      status={marks[s.id]?.[p.participant_id] ?? null}
                      onSet={(st) => setCell(s.id, p.participant_id, st)}
                      locked={locked}
                    />
                  </Box>
                ))}
