 ...
 *
 * The abbreviation itself comes from the tz database via `lib/timezones`; this
 * only decides whether the value it was handed is a zone at all.
 */
export function timezoneLabel(tz, at) {
  const on = parseWallClock(at);
  if (!tz || !on) return "";
  // A feed value like "PHT" or "CST" is already a display abbreviation, not an
  // IANA zone, and handing it to Intl is unsafe in both directions: Intl REJECTS
  // "PHT" (so the time renders with no zone at all) and silently ACCEPTS "CST",
  // "EST", "PST" and "MST" as the legacy *US* zones — labelling a Beijing
  // session "CDT". The abbreviation is already the label, so echo it back.
  if (!tz.includes("/")) return tz;
  return zoneAbbreviation(tz, on);
}
