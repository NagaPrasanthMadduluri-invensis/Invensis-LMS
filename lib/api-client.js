...
export class ApiError extends Error {
  /**
   * `data` is the full parsed error body. Some endpoints answer a refusal with
   * fields the caller has to act on rather than just display — e.g. a 409
   * carrying `code` and a count, which a confirmation dialog reads to offer an
   * override. `errors` stays the field-level validation map.
   */
  constructor(message, status, errors = null, data = null) {
    super(message);
    this.name = "ApiError";
    this.status = status;
    this.errors = errors;
